...
void ServerConnection::receive()
{
    ...
    QByteArray out_msg("ws connection ...");
    bool snd = m_p_pub_sock->send(&out_msg, out_msg.size());
    qDebug() << "send msg:" << snd;
}
...
void ServerConnection::setPullAddr(QByteArray pull_addr)
{
    ...
    emit pullAddrChanged(pull_addr);
}
...
void ServerConnection::setSenderId(QByteArray sender_id)
{
    ...
    emit senderIdChanged(sender_id);
}
